package raytracer.scene;

import raytracer.camera.PerspectiveCamera;
import raytracer.geometry.World;
import raytracer.geometry.ZAxisAlignedCylinder;
import raytracer.light.PointLight;
import raytracer.material.PhongMaterial;
import raytracer.math.Point3;
import raytracer.math.Vector3;
import raytracer.texture.Color;

/**
 * Created by ok on 26.11.15.
 */
public class Torus extends RtScene {
    public Torus() {
        cam = new PerspectiveCamera(new Point3(5, 5, 5), new Vector3(-1, -1, -1), new Vector3(0, 0, 1), Math.PI / 4);
        world = new World(new Color(0, 0, 0), new Color(0.2, 0.2, 0.2));
        world.addGeometry(new raytracer.geometry.Torus(3, 1, new PhongMaterial(new Color(0, 1, 0), new Color(1, 1, 1), 64)));

        world.addLight(new PointLight(new Color(0.5, 0.5, 0.5), new Point3(5, 5, 0)));
    }
}
